...
sustain = False
playingsounds = []
globalvolume = 10 ** (-9.0/20)  # -12dB default global volume
globaltranspose = 0
...
def MidiMessagePrint(message):
    MidiStatus = '{0:08b}'.format(message[0])
    MidiData1 = '{0:08b}'.format(message[1])
    if len(message) > 2:
        MidiData2 = '{0:08b}'.format(message[2])
    else:
        MidiData2 = '........'
    print('Message is: ', MidiStatus, MidiData1, MidiData2)
...
#    MidiMessagePrint(message) #Enable this to see Midi messages printed to std out
    ...
    elif (messagetype == 11) and (note == 48) and (preset > 0):  # B1, select down, unless already at preset 0
        preset -= 1
        LoadSamples()

    elif (messagetype == 11) and (note == 49) and (preset < 15):  # B2, select up, unless already at preset 15
        preset += 1
        LoadSamples()

    elif (messagetype == 11) and (note == 50):  # Adjust Volume
	DBVolume = (velocity // 5) - 30 # Calculating new Decibel setting
        globalvolume = 10 ** (float(DBVolume)/20) # Setting GlobalVolume (10 ** (Decibel/20)
